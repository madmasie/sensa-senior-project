...
\node[ext, left=6.0cm of out]
      (alerts) {Haptic Alert\\\normalfont\normalsize Vibration motor\\ \normalfont\normalsize Piezo buzzer};
\node[ext, minimum height=3.2cm, above=2.0cm of alerts]
      (dash)   {Dashboard \\\normalfont\normalsize BLE, WIFI app};
\node[ext, minimum height=3.2cm, above=2.0cm of dash]
      (idle)   {Power Saver \\\normalfont\normalsize sub-Nyquist sampling \\\normalfont\normalsize Low Power Mode};
...
\draw[flow] (dash.north) -- (idle.south);
\draw[flow] (idle.north) -- (pwr.south);
\end{tikzpicture}%
}
...
\begin{itemize}\setlength\itemsep{0.25em}
  ...
  \item \textbf{Transfer learning} fine-tunes the local Sensa data with a PurpleAir reference 
\end{itemize}
\vspace{0.3em}
\centering
...
\includegraphics[width=\linewidth]{img/predictions.png}
\centering
